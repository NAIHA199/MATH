...
<body>
  ...
  <main>
    ...
    @guest
      <p>Vui lòng đăng nhập hoặc đăng ký để bắt đầu học toán!</p>
    @else
      <a href="{{ route('dashboard') }}" class="btn" style="display: inline-block; margin-top: 20px; text-decoration: none;">
        Vào Dashboard
      </a>
    @endguest
  </main>
  ...
  <script>
    ...
    @if(old('role'))
      document.addEventListener('DOMContentLoaded', function() {
        const role = '{{ old("role") }}';
        const roleElement = document.querySelector(`[onclick*="${role}"]`);
        if (roleElement) {
          selectRole(roleElement, role);
        }
      });
    @endif
  </script>
